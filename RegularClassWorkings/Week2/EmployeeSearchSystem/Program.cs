...
class Employee
{
    ...
    public Employee(int id, string name, string department, string designation, int experience, double salary, string city)
    {
        ...
        Experience = experience; Salary = salary; City = city;
    }
...
class Program
{
    ...
    static void Main()
    {
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case 2: Console.Write("Enter Employee ID: "); LinearSearch(Convert.ToInt32(Console.ReadLine())); break;
                case 3: Console.Write("Enter Employee ID: "); BinarySearch(Convert.ToInt32(Console.ReadLine())); break;
                case 4: Console.Write("Enter Employee Name: "); SearchByName(Console.ReadLine()); break;
                case 5: Console.Write("Enter Department: "); SearchByDepartment(Console.ReadLine()); break;
                case 6: Console.Write("Enter City: "); SearchByCity(Console.ReadLine()); break;
                case 7: Console.Write("Enter Minimum Experience: "); SearchByExperience(Convert.ToInt32(Console.ReadLine())); break;
                case 8:
                    Console.Write("Enter Minimum Salary: "); double min = Convert.ToDouble(Console.ReadLine());
                    Console.Write("Enter Maximum Salary: "); double max = Convert.ToDouble(Console.ReadLine());
                    SearchBySalary(min, max); break;
                case 9: Console.WriteLine("Thank You!"); return;
                default: Console.WriteLine("Invalid Choice."); break;
            }
        }
    }

    static void DisplayAll() { foreach (Employee emp in employees) emp.Display(); }

    static void LinearSearch(int id)
    {
        foreach (Employee emp in employees)
            if (emp.Id == id) { Console.WriteLine("\nEmployee Found:"); emp.Display(); return; }
    ...
    static void BinarySearch(int id)
    {
        ...
        {
            int mid = (low + high) / 2;
            if (sorted[mid].Id == id) { Console.WriteLine("\nEmployee Found:"); sorted[mid].Display(); return; }
            else if (id < sorted[mid].Id) high = mid - 1;
            else low = mid + 1;
        }
        Console.WriteLine("Employee Not Found.");
    }
    ...
    static void SearchByName(string name)
    {
        bool found = false;
        foreach (Employee emp in employees)
            if (emp.Name.ToLower().Contains(name.ToLower())) { emp.Display(); found = true; }
    ...
    static void SearchByDepartment(string department)
    {
        bool found = false;
        foreach (Employee emp in employees)
            if (emp.Department.Equals(department, StringComparison.OrdinalIgnoreCase)) { emp.Display(); found = true; }
        if (!found) Console.WriteLine("No Employees Found.");
    }
    ...
    static void SearchByCity(string city)
    {
        bool found = false;
        foreach (Employee emp in employees)
            if (emp.City.Equals(city, StringComparison.OrdinalIgnoreCase)) { emp.Display(); found = true; }
        if (!found) Console.WriteLine("No Employees Found.");
    }

    static void SearchByExperience(int experience)
    {
        bool found = false;
        foreach (Employee emp in employees)
            if (emp.Experience >= experience) { emp.Display(); found = true; }
        if (!found) Console.WriteLine("No Employees Found.");
    }

    static void SearchBySalary(double minSalary, double maxSalary)
    {
        bool found = false;
        foreach (Employee emp in employees)
            if (emp.Salary >= minSalary && emp.Salary <= maxSalary) { emp.Display(); found = true; }
        if (!found) Console.WriteLine("No Employees Found.");
    }
}
